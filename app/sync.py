...
async def _bring_login(settings: Settings) -> Optional[BringAuth]:
    ...
    if not token or not user_uuid or not list_uuid:
        _log_event(settings, "ERROR", "log.bring_login_response_incomplete", data)
        return None
    ...
    async with SYNC_LOCK:
        ...
        for item in items:
            ...
            if ok and item_id:
                ...
                if done:
                    _log_event(settings, "INFO", "log.mealie_marked_done", {
                        "itemId": item_id,
                        "name": name,
                    })
                else:
            ...
            if ok:
                _log_event(settings, "INFO", "log.bring_transfer_success", {
                    "itemId": item_id,
                    "name": name,
                    "note": note,
                })
            else:
                _log_event(settings, "ERROR", "log.bring_transfer_failed", {
                    "itemId": item_id,
                    "name": name,
                })
