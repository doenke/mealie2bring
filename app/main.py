...
def _translate_log_entry(entry: dict) -> dict:
...
async def dashboard(request: Request):
    ...
    entries = [_translate_log_entry(entry) for entry in load_log_entries(settings)]
    sync_started_label = t("log.sync_started", {})
    last_sync_entry = next(
        (
            ...
            if entry.get("type") == "event"
            and (
                entry.get("message_key") == "log.sync_started"
                or entry.get("message") == sync_started_label
            )
        ),
        None,
    )
